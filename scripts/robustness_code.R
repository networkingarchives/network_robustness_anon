...
emlo_results = robustness_test(emlo_network, nsim = 20)  
spo_results = robustness_test(spo_network, nsim = 20)
bcc_results = robustness_test(bcc_network, nsim = 20)  


save(bcc_results, file = 'bcc_results')

rbind(emlo_results %>% 
        mutate(dataset = 'emlo'), spo_results %>% 
        mutate(dataset = 'spo'), bcc_results %>% 
        mutate(dataset = 'bcc')) %>% filter(!is.na(run)) %>% 
  ggplot() + 
  geom_point(aes(x = sample, y= cor, color = metric)) + 
  facet_grid(type~dataset~metric) + coord_cartesian(ylim = c(0,1))
...
bcc_nodes = robustness_test_nodes(bcc_network, nsim = 20)
spo_nodes = robustness_test_nodes(spo_network, nsim = 20)
emlo_nodes = robustness_test_nodes(emlo_network, nsim = 20)
