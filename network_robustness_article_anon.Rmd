...
>They did not fear that he would abandon them as long as his books and writings were not sent from thence. Thus they have lost both his manuscripts and their own records &c which might have deserved an exception from their general resolution of sending nothing out of Lesna.^[John Pell to Samuel Hartlib. Zürich, 27 July 1656. BL Add MS 4280, 37r-v. Digital version of printed transcription consulted, in Jana Amosa Komenského korrespondence. Ed. Adolf Patera. Prague: 1892. No. 159. Found in Early Modern Letters Online [http://tinyurl.com/yaumwclk]]
...
```{r example, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Two example comparisons of eigenvector centrality scores, with 80% and 50% of letters removed at random.", fig.height=2.2, fig.width=4.5}
set.seed(1234)
spo_raw = read_delim('fromto_all_place_mapped_stuart_sorted', delim = '\t', col_names = F )
...
nodes = graphmatr %>% 
  as_tbl_graph() %>% 
  mutate(degree =centrality_degree(mode = 'total')) %>% 
  filter(degree>1) %>% activate(nodes) %>% as_tibble() %>% pull(name)
...
network_filtered_f = spo_raw %>% 
  ...
  mutate(folio = str_remove(X8, "f\\.[0-9]{1}")) %>% 
  filter(!is.na(folio)) %>% 
  mutate(folio = trimws(folio, which = 'both')) %>% filter(X1 %in% nodes |X2 %in% nodes)
graphmatr = graph_from_data_frame(network_filtered_f %>%
distinct(.[1], .[2]), directed = T)
...
temp.stats_50 = igraph::eigen_centrality(sub.graphmatr)['vector'][[1]] %>% tibble::enframe()
temp.stats_50$full = bw.g[temp.stats_50$name]
...
sample_edges = network_filtered_f %>%
  ...
  distinct(.[1], .[2])
sub.graphmatr = graph_from_data_frame(sample_edges)
temp.stats_20 = igraph::eigen_centrality(sub.graphmatr)['vector'][[1]] %>% tibble::enframe()
temp.stats_20$full = bw.g[temp.stats_20$name]



p20 = cor.test(temp.stats_20$value, temp.stats_20$full)$estimate['cor']
p50 = cor.test(temp.stats_50$value, temp.stats_50$full)$estimate['cor']

temp.stats_20 = temp.stats_20 %>% mutate(remaining = paste0('20% Remaining ', "(r of ", round(p20,3), ", p<.01)"))

temp.stats_50 = temp.stats_50 %>% mutate(remaining = paste0('50% Remaining ', "(r of ",round(p50,3), ", p<.01)"))

rbind(temp.stats_20, temp.stats_50) %>% ggplot() + 
  geom_point( aes(x = value, y = full), size = .5, alpha  = .6) + 
  facet_wrap(~remaining) + 
  theme_bw() + 
  theme(text = element_text(family = 'Times')) + 
...
robustness_results %>% 
 ...
 # select(-dataset) %>% 
  ...
  kbl(escape = F, booktabs = T, col.names = c('', 'Portion removed', 'Mean $\\rho$', paste0('SD',footnote_marker_symbol(1)), 'Mean $\\rho$', 'SD', 'Mean $\\rho$', 'SD', 'Mean $\\rho$', 'SD'), caption = "Summary statistics for the removal correlations at twenty and fifty percent removed, showing the mean Spearman's rho ($\\rho$) and standard deviation across forty iterations.") %>% 
...
Historical network data (as indeed most network data - see Introduction) is always incomplete: this may be because of gaps in archives, the lack of digitisation, or simply because most communication is face-to-face and therefore leaves little record. There is a subset of more mundane 'knowable' missing archival data, related in the first instance to its materiality as well as its editorial and digital afterlives: the missing data within it is from lost letters, burned books, uncalendared State papers, undigitised editions, uncrackable ciphers, and so forth which we can model and understand its impact on findings. Scholars working on correspondence have sometimes manually catalogued missing letters, and some of this data has been added to the EMLO database.^[See _The Correspondence of Henry Oldenburg_, ed. A. R. Hall and M. B. Hall, 13 vols (Madison, WI: University of Wisconsin Press; London: Mansel; London: Taylor & Francis, 1965–86) and for a specific example from EMLO https://tinyurl.com/y8lfdvha] This reconstruction is valuable, which in itself brings more accuracy to quantitative historical network studies, but it is difficult. _Modelling_ the effect of missing letters, even if one cannot grasp their precise existence, is an enterprise which, as we have shown, is less time-consuming and can help to deal with the problem in the absence of reconstructed archives.
...
library(png)
library(grid)
img <- readPNG("Screenshot 2020-09-15 at 10.03.06.png")
 grid.raster(img)
```
